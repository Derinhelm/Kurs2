...
class Node:
    def __init__(self, id1, dom1, feat1, text1, textNorm1, morphs1):
        ...
        # список Morph, удовлетворяющих тегу корпуса

        # replace(".","") для сокращений
class Inserter:
    def __init__(self, mf, mt, df, dt):
        self.mainMorphs = mf #список Morph
        self.mainText = mt
        self.depMorphs = df #список Morph
        self.depText = dt
        self.count = 1
    def __eq__(self, other):
        return self.mainMorphs == other.mainMorphs and self.depMorphs == other.depMorphs and\
...
def parseCorporaTag(tagCorpora):
    ...
    for curParam in tagCorpora:
        if curParam == 'ИМ':
            checkFuns.append(lambda m: m.s_cl  == 'nominative')
        elif curParam == 'РОД':
            checkFuns.append(lambda m: m.s_cl  == 'genitive')
        elif curParam == 'ДАТ':
            checkFuns.append(lambda m: m.s_cl  == 'dative')
        elif curParam == 'ВИН':
            checkFuns.append(lambda m: m.s_cl  == 'accusative')
        elif curParam == 'ТВОР':
            checkFuns.append(lambda m: m.s_cl  == 'instrumental')
        elif curParam == 'ПР':
            checkFuns.append(lambda m: m.s_cl  == 'prepositional')
        elif curParam == 'ПАРТ':
            checkFuns.append(lambda m: m.s_cl  == 'genitive')
        elif curParam == 'МЕСТН':
            checkFuns.append(lambda m: m.s_cl  == 'prepositional')
...
def getParseByPymorphy(curWord, curTagCorpora, curNormalForm, arrParse):
    ...
    if impFeatures == None: # слово типа NID, 'as-sifr'
        return None
    #print(curWord, curTagCorpora, curNormalForm, arrParse)
    notImpFeat = copy.copy(Morph.names) - impFeatures
    goodParsePymorphy = []
...
def parseXML(nameFile, morph):
    ...
    for item in parent:
        ...
        for child in item.getElementsByTagName('W'):
            if ('0' <= child.getAttribute('LEMMA')[0] <= '9'):
                newNode = Node(child.getAttribute('ID'), child.getAttribute('DOM'), \
                               None, None, None, None)
...
#<W DOM="5" FEAT="V СОВ ИЗЪЯВ ПРОШ МН" ID="9" LEMMA="ОТВОДИТЬ" LINK="сочин" NODETYPE="FANTOM"/>
                ...
                if realMorphs != None:
                    newNode = Node(id1, dom, feat, word, normWord, realMorphs)
                else:
                    newNode = Node(child.getAttribute('ID'), child.getAttribute('DOM'), \
                                   None, None, None, None)
                    print(i, word)
    ...
    for curSentence in arrayParseSentences:
        for curWord in curSentence:
            ...
            if curWord.dom != '_root':
                ...
                if curWord.text and mainWord.text:
                    newInserter = Inserter(mainWord.arrMorphs, mainWord.textNorm, \
                                       curWord.arrMorphs, curWord.textNorm)
...
def insertPattern3(con, mainMorphsNumber, depMorphsNumber, \
    ...
    params = (mainMorphsNumber, depMorphsNumber, \
            mainWordNumber, depWordNumber)
    ...
    con.commit()
    cursor.close()
def insertPattern2(con, mainMorphsNumber, depMorphsNumber, \
                  mainWordNumber):
    cursor = con.cursor()
...
def insertPattern1(con, mainMorphsNumber, depMorphsNumber):
    cursor = con.cursor()
...
def insertPattern(con, mainMorphsNumber, depMorphsNumber, \
                  mainWordNumber, depWordNumber, mainMorph, depMorph):
    print(mainMorphsNumber, depMorphsNumber, \
                  mainWordNumber, depWordNumber)
# пока нет проверки на предлог и глагол....
    ...
    if mainMorph.s_cl == 'preposition':
        insertPattern2(con, mainMorphsNumber, depMorphsNumber, \
                       mainWordNumber)
        insertPattern1(con, mainMorphsNumber, depMorphsNumber)
    elif depMorph.s_cl == 'preposition':
        insertPattern3(con, mainMorphsNumber, depMorphsNumber, \
                       mainWordNumber, depWordNumber)
    else:
        insertPattern3(con, mainMorphsNumber, depMorphsNumber, \
                      mainWordNumber, depWordNumber)
...
def insertAllPairs(con, mainWord, mainMorphs, depWord, depMorphs):
# mainWord - массив начальных форм
# depWord - массив начальных форм
# mainMorphs - массив морфов главного слова
# depMorphs - массив морфов зависимого слова

# массив номеров слова в таблице word(мб несколько в дальнейшем)
# пока массив из одного элемента
    mainWordNumbers = map(str, findOrInsertWord(con, mainWord))

    depWordNumbers = map(str, findOrInsertWord(con, depWord))
    mainMorphsNumbers = []
    for curMainMorph in mainMorphs:
        mainMorphsNumbers += map(str, findOrInsertMorph(con, curMainMorph))
    depMorphsNumbers = []
    for curDepMorph in depMorphs:
        depMorphsNumbers += map(str, findOrInsertMorph(con, curDepMorph))
    for curMainWordNumber in mainWordNumbers:
        for curDepWordNumber in depWordNumbers:
            for i in range(len(mainMorphsNumbers)):
                curMainMorphsNumber = mainMorphsNumbers[i]
                curMainMorph = mainMorphs[i]
                for j in range(len(depMorphsNumbers)):
                    curDepMorphsNumber = depMorphsNumbers[j]
                    curDepMorph = depMorphs[j]
                    insertPattern(con, curMainMorphsNumber, \
                        curDepMorphsNumber,  curMainWordNumber, curDepWordNumber, curMainMorph, curDepMorph)


def insertManyInserter(con, manyInserter):
    for i in range(len(manyInserter)):
        print(manyInserter[i].mainText, manyInserter[i].depText)
        mainWord = manyInserter[i].mainText
        mainMorphs = manyInserter[i].mainMorphs
        depWord = manyInserter[i].depText
        depMorphs = manyInserter[i].depMorphs

        # как-то вставляем все в базу
        # insertModels(con, mainNormalForm, mainVariantsMorph,\
        #            depNormalForm, depVariantsMorph)
        insertAllPairs(con, mainWord, mainMorphs, \
                       depWord, depMorphs)
...
for i in range(50):
    ...
    print(name)
    allName = "all/" + name
    manyInserter = createInserters(allName, morph)
    insertManyInserter(con, manyInserter)
