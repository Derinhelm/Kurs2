...
class Node:
    def __init__(self, id1, dom1, arrIns):
        self.id = id1
        self.dom = dom1
        self.arrInserts = arrIns
        # список, каждый элемент - кортеж из
        # 1.Morph, удовлетворяющих тегу корпуса
        # 2.начальная форма слова(полученная с помощью pymotphy2)
class Inserter:
    def __init__(self, m, d):
        self.mainInserts = m         # список, каждый элемент - кортеж из
        # 1.Morph, удовлетворяющих тегу корпуса
        # 2.начальная форма слова(полученная с помощью pymotphy2)
        self.depInserts = d         # список, каждый элемент - кортеж из
        # 1.Morph, удовлетворяющих тегу корпуса
        # 2.начальная форма слова(полученная с помощью pymotphy2)
        ...
        for curWord in curSentence:
            if curWord.dom != '_root':
                ...
                if curWord.arrInserts and mainWord.arrInserts:
                    newInserter = Inserter(mainWord.arrInserts, curWord.arrInserts)
...
# пока нет проверки на предлог и глагол....
    ...
    if mainMorph.s_cl == 'preposition':
        print(mainMorphNumber, depMorphNumber, \
              mainWordNumber, depWordNumber)
...
# пока массив из одного элемента
    for curMain in mainInserts:
        for curDep in depInserts:
            ...
            mainMorph = curMain[0]
            depMorph = curDep[0]
            mainWordNumber = str(findOrInsertWord(con, mainWord))
            depWordNumber = str(findOrInsertWord(con, depWord))
            mainMorphNumber = str(findOrInsertMorph(con, mainMorph))
            depMorphNumber = str(findOrInsertMorph(con, depMorph))
            insertPattern(con, mainMorphNumber, \
                    depMorphNumber,  mainWordNumber, depWordNumber, mainMorph, depMorph)


def insertManyInserter(con, manyInserter):
    for i in range(len(manyInserter)):
        # вставляем все в базу
        insertAllPairs(con, manyInserter[i].mainInserts, manyInserter[i].depInserts)
