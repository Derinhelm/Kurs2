...
def onMouseClickTree(event, dictParsePoints, pos):
    ...
    if event.dblclick:
        ...
        parse_point = dictParsePoints[near_point]
        parse_point_visualize(parse_point, parse_point.__repr__())
        event.key = None
        plt.show()

def parse_tree_visualize(graph, dictParsePoints, title):
    ...
    fig.canvas.mpl_connect('button_press_event', lambda event: onMouseClickTree(event, dictParsePoints, pos))
...
def parse_point_visualize(parse_point, name):
        ...
        y_margin = (y_max - y_min) * 0.25
        plt.ylim(y_min - y_margin, y_max + y_margin)
        fig.canvas.mpl_connect('button_press_event', lambda event: onMouseClickParsePoint(event, parse_point, pos))
        nx.draw(graph, pos, with_labels=True, arrows=False, node_size=1, horizontalalignment='center',
